 ...
 * Every time the configuration is modified (e.g. blacklist/whitelist modification),
 * the {@link #load(CommandSender)} should be triggered to reload the required listeners.
 * <p>
...
public final class BanListener {
    ...
    public void load(@NotNull final CommandSender sender) {
        ...
        if (blacklist.contains(BanAction.BOOKEDIT) || whitelist) {
            registerEvent(PlayerEditBookEvent.class, (li, event) -> {
                ...
                if (api.isBanned(e.getPlayer(),
                        Utils.getItemInHand(e.getPlayer()),
                        true,
                        BanAction.BOOKEDIT)) {
                    e.setCancelled(true);
        ...
        if (blacklist.contains(BanAction.BREW) || whitelist) {
            registerEvent(BrewEvent.class, (li, event) -> {
                ...
                final ItemStack ingredient = e.getContents().getIngredient() == null ? null : e.getContents().getIngredient().clone();
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        if (api.isBanned(e.getBlock().getWorld(), item, BanAction.BREW)) {
                            if (!inv.getViewers().isEmpty()) {
                                if (!api.isBanned((Player) inv.getViewers().get(0),
                                        e.getBlock().getLocation(),
                                        item,
                                        true,
                                        BanAction.BREW)) {
        ...
        if (blacklist.contains(BanAction.DROPS) || whitelist) {
            if (MinecraftVersion.v13OrMore) {
                registerEvent(BlockDropItemEvent.class, (li, event) -> {
                    ...
                    final ItemStack itemInHand = Utils.getItemInHand(e.getPlayer());
                    e.getItems().removeIf(item -> api.isBanned(e.getPlayer(),
                            e.getBlock().getLocation(),
                            item.getItemStack(),
                            true,
                            BanAction.DROPS,
                ...
                registerEvent(BlockBreakEvent.class, (li, event) -> {
                    ...
                    final BlockBreakEvent e = (BlockBreakEvent) event;
                    final ItemStack itemInHand = Utils.getItemInHand(e.getPlayer());
                    if (e.getBlock().getDrops(itemInHand).stream().anyMatch(item -> api
        ...
        if (blacklist.contains(BanAction.ENCHANT) || whitelist) {
            registerEvent(EnchantItemEvent.class, (li, event) -> {
                ...
                if (api.isBanned(e.getEnchanter(),
            ...
            if (MinecraftVersion.v9OrMore) {
                ...
                registerEvent(PrepareAnvilEvent.class, (li, event) -> {
                    ...
                    if (api.isBanned(p,
                            ...
                            true,
                            BanAction.ENCHANT,
                            new BanData(BanDataType.ENCHANTMENT, EnchantmentWrapper.from(enchants)))) {
            ...
            registerEvent(EntityDeathEvent.class, (li, event) -> {
                ...
                if (killer != null) {
                    e.getDrops().removeIf(item -> {
                        ...
                        return api.isBanned(killer,
                                e.getEntity().getLocation(),
                                item,
                                true,
                                BanAction.ENTITYDROP,
                    ...
                    e.getDrops().removeIf(item -> {
                        ...
                        return api.isBanned(e.getEntity().getWorld(),
                                item,
                                BanAction.ENTITYDROP,
                                new BanData(BanDataType.ENTITY, e.getEntity().getType()));
                    });
        ...
        if (blacklist.contains(BanAction.GLIDE) || whitelist) {
            if (!MinecraftVersion.v9OrMore) {
                ...
                registerEvent(org.bukkit.event.entity.EntityToggleGlideEvent.class, (li, event) -> {
                    ...
                    final EntityEquipment entityEquipment = p.getEquipment();
                    if (entityEquipment == null) {
                        return;
                    }
                    final ItemStack item = entityEquipment.getChestplate();
                    if (item == null) {
                        return;
                    }
                    if (api.isBanned(p, item, true, BanAction.GLIDE)) {
                        ...
                        plugin.getServer().getScheduler().runTask(plugin, () -> {
                            ...
                            // Already removed?
                            final ItemStack chestPlateItem = entityEquipment.getChestplate();
                            if (Utils.isNullOrAir(chestPlateItem)) {
                                return;
                            }

                            // Put chest plate in inventory
                            final int freeSlot = p.getInventory().firstEmpty();
                            if (freeSlot == -1) { // no empty space, dropping it
                                p.getWorld().dropItemNaturally(p.getLocation(), chestPlateItem);
                                entityEquipment.setChestplate(null);
                                return;
                            }

                            // Putting it back in inventory
                            p.getInventory().setItem(freeSlot, chestPlateItem);
                            entityEquipment.setChestplate(null);
                        });
                    }
        ...
        if (blacklist.contains(BanAction.HOLD) || whitelist) {
            ...
            registerEvent(InventoryDragEvent.class, (li, event) -> {
                ...
                if (e.getInventorySlots().contains(p.getInventory().getHeldItemSlot()) &&
                        api.isBanned(p, item, true, BanAction.HOLD)) {
                    e.setCancelled(true);
                }
            }, priority.contains(BanAction.HOLD));
            ...
            // Pickup
            // MC>=1.12: EntityPickupItemEvent
            // MC<1.12: PlayerPickupItemEvent
            if (MinecraftVersion.v12OrMore) {
        ...
        if (blacklist.contains(BanAction.INVENTORYCLICK) || whitelist) {
            registerEvent(InventoryClickEvent.class, (li, event) -> {
                ...
                if (api.isBanned((Player) e.getWhoClicked(),
                        item,
                        true,
                        BanAction.INVENTORYCLICK,
                        new BanData(BanDataType.INVENTORY_FROM, inv.getType()))) {
                    e.setCancelled(true);
                }
        ...
        if (blacklist.contains(BanAction.PICKUP) || whitelist) {
            // Pickup cooldown map clearing on player quit
            registerEvent(PlayerQuitEvent.class, (li, event) ->
                    plugin.getUtils().getMessagesCooldown()
                            .remove(((PlayerQuitEvent) event).getPlayer().getUniqueId()), priority.contains(BanAction.PICKUP));
            ...
            if (MinecraftVersion.v12OrMore) {
                registerEvent(org.bukkit.event.entity.EntityPickupItemEvent.class, (li, event) -> {
                    ...
                    if (api.isBanned((Player) e.getEntity(),
                            e.getItem().getLocation(),
                            e.getItem().getItemStack(),
                            true,
                            BanAction.PICKUP)) {
        ...
        if (blacklist.contains(BanAction.PLACE) || whitelist) {
            registerEvent(BlockPlaceEvent.class, (li, event) -> {
                ...
                if (api.isBanned(e.getPlayer(),
                        ...
                        true,
                        BanAction.PLACE,
                        new BanData(BanDataType.MATERIAL, e.getBlockAgainst().getType()))) {
                    e.setCancelled(true);
                    if (!MinecraftVersion.v12OrMore) {
                        e.getPlayer().updateInventory();
                    }
        ...
        if (blacklist.contains(BanAction.RENAME) || whitelist) {
            registerEvent(InventoryClickEvent.class, (li, event) -> {
                ...
                if (inv == null || inv.getType() != InventoryType.ANVIL || e.getRawSlot() != 2) {
                    return;
                }
                ...
                if (!itemName.equals(resultName) &&
                        api.isBanned((Player) e.getWhoClicked(),
                                item,
                                true,
                                BanAction.RENAME)) {
                    e.setCancelled(true);
            ...
            // Prevent usage of a rename command
        ...
        if (blacklist.contains(BanAction.SMELT) || whitelist) {
            registerEvent(FurnaceSmeltEvent.class, (li, event) -> {
                ...
                if (api.isBanned(blockState.getWorld(), item, BanAction.SMELT)) {
                    ...
                    if (api.isBanned((Player) holder.getInventory().getViewers().get(0),
        ...
        if (blacklist.contains(BanAction.SMITH) || whitelist) {
            if (!MinecraftVersion.v16OrMore) {
                ...
                registerEvent(PrepareSmithingEvent.class, (ll, event) -> {
                    ...
                    final HumanEntity humanEntity = e.getViewers().get(0);
                    if (!(humanEntity instanceof Player)) {
                        return;
                    }
                    if (api.isBanned((Player) humanEntity, item, true, BanAction.SMITH)) {
                        e.setResult(null);
                    }
        ...
        if (blacklist.contains(BanAction.TRANSFER) || whitelist) {
            ...
            registerEvent(InventoryClickEvent.class, (li, event) -> {
                ...
                if (top.getType() != InventoryType.CRAFTING
                        && e.getClick() == ClickType.DOUBLE_CLICK) { // Trying to get all items for a banned one?
                    ...
                    if (!Utils.isNullOrAir(item) &&
                            api.isBanned(player,
                                    ...
                                    true,
                                    BanAction.TRANSFER,
                                    new BanData(BanDataType.INVENTORY_FROM, bottom.getType()),
                    ...
                    if (e.isShiftClick() && e.getAction() == InventoryAction.MOVE_TO_OTHER_INVENTORY) {
                        ...
                        if (!Utils.isNullOrAir(item) &&
                                api.isBanned(player,
                                        ...
                                        true,
                                        BanAction.TRANSFER,
                                        new BanData(BanDataType.INVENTORY_FROM, top.getType()),
                        ...
                        if (!Utils.isNullOrAir(cursorItem) && api.isBanned(player,
                                cursorItem,
                                true,
                                BanAction.TRANSFER,
                                new BanData(BanDataType.INVENTORY_FROM, bottom.getType()),
            ...
            registerEvent(InventoryDragEvent.class, (li, event) -> {
                ...
                if (e.getView().getTopInventory().getType() == InventoryType.CRAFTING ||
                        Utils.isNullOrAir(e.getOldCursor())) {
                    return;
                }
            ...
            if (plugin.getBanConfig().getConfig().getBoolean("actions.transfer.hoppers-block")) {
                ...
                registerEvent(InventoryPickupItemEvent.class, (li, event) -> {
                    ...
                    if (api.isBanned(e.getItem().getWorld(),
                            e.getItem().getItemStack(),
                            BanAction.TRANSFER,
        ...
        if (blacklist.contains(BanAction.WEAR) || whitelist) {
            registerEvent(InventoryClickEvent.class, (li, event) -> {
                ...
                if (e.getRawSlot() >= 5 && e.getRawSlot() <= 8) {
                    plugin.getServer().getScheduler().runTask(plugin, () ->
                            plugin.getUtils().checkPlayerArmors((Player) e.getWhoClicked()));
                    return;
                }
            ...
            registerEvent(PlayerChangedWorldEvent.class, (li, event) -> {
                ...
                plugin.getServer().getScheduler().runTask(plugin, () -> plugin.getUtils().checkPlayerArmors(e.getPlayer()));
            }, priority.contains(BanAction.WEAR));
                ...
                if (hook == null) {
                    ...
                    registerEvent(PlayerMoveEvent.class, (li, event) -> {
                        ...
                        if (!hook.getStandingRegions(from).equals(hook.getStandingRegions(to))) {
                            plugin.getServer().getPluginManager().callEvent(new PlayerRegionChangeEvent(e.getPlayer()));
                        }
                    }, priority.contains(BanAction.WEAR));
                    ...
                    registerEvent(PlayerRegionChangeEvent.class, (li, event) -> {
                        ...
                        plugin.getServer().getScheduler().runTask(plugin, () ->
                                plugin.getUtils().checkPlayerArmors(e.getPlayer()));
                    }, priority.contains(BanAction.WEAR));
                }
            }

            // Scanner?
            plugin.getUtils().getWearScanner().setEnabled(
                    plugin.getBanConfig().getConfig().getBoolean("actions.wear.scanner"));
        }
    }

    /**
     * Registering a required event.
     *
     * @param c             the event class
     * @param eventExecutor the event executor
     * @param priority      if the event should have maximum priority
     */
    private void registerEvent(@NotNull final Class<? extends Event> c,
                               @NotNull final EventExecutor eventExecutor,
                               final boolean priority) {
        plugin.getServer().getPluginManager().registerEvent(c,
                listener,
                (priority ? EventPriority.LOWEST : EventPriority.NORMAL),
                eventExecutor,
                plugin,
                !priority);
        activated++;
     ...
     *
     * @return the amount of events registered
     */
