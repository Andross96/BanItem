...
public final class BanAnimation {
     ...
     */
    public BanAnimation(@NotNull final BanUtils utils, @NotNull final CommandSender sender, @NotNull final FileConfiguration config) {
        // Sound
        ...
        if (soundEnabled) {
            try {
                ...
                sender.sendMessage(utils.getPrefix() + utils.color("&cInvalid sound parameters set in config.yml."));
            }
        }
        // Particle
        ...
        if (particleEnabled && BanVersion.v9OrMore) {
            try {
                ...
                sender.sendMessage(utils.getPrefix() + utils.color("&cInvalid particle parameters set in config.yml."));
            }
        }
    }
